...
const ResetPassword: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTimeout(() => router.push("/reset-successful"), 1000)
    } catch (error) {
          ...
          <div className="flex flex-1 items-center justify-center">
            <div className="w-full max-w-[500px] rounded-lg max-sm:w-[95%] max-sm:p-4 md:bg-[#FDFCFC]">
              <div className="t flex w-full flex-col justify-center">
                <p className="text-center text-4xl font-semibold text-[#212121]">Set a new Password</p>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-10 mt-4 flex w-full flex-col gap-2">
                  <p className="font-semibold text-[#212121]">Your password must contain:</p>
                  <div className="flex items-center gap-2">
                    <CheckIcon />
                    <p className={` text-xs ${passwordChecks.minLength ? "text-[#198754]" : "text-[#6C757D]"}`}>
                      Minimum 8 characters
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckIcon />
                    <p className={` text-xs ${passwordChecks.hasUppercase ? "text-[#198754]" : "text-[#6C757D]"}`}>
                      At least one uppercase letter
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckIcon />
                    <p className={` text-xs ${passwordChecks.hasNumber ? "text-[#198754]" : "text-[#6C757D]"}`}>
                      At least one number
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckIcon />
                    <p className={` text-xs ${passwordChecks.hasSpecialChar ? "text-[#198754]" : "text-[#6C757D]"}`}>
